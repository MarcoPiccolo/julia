...
static void jl_rebuild_methtables(arraylist_t* MIs, htable_t* mtables)
{
    ...
    for (i = 0; i < MIs->len; i++) {
        ...
        if (!ptrhash_has(mtables, old_mt))
            ptrhash_put(mtables, old_mt, jl_new_method_table(old_mt->cache->name, old_mt->cache->module));
        jl_methtable_t *mt = (jl_methtable_t*)ptrhash_get(mtables, old_mt);
        size_t world =  jl_atomic_load_acquire(&jl_world_counter);
        jl_value_t *lookup = jl_methtable_lookup(m->sig, world); // TODO: this is quite wrong: use do_typemap_search instead to avoid incorrect serialization
