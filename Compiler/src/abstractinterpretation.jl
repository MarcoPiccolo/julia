...
function find_union_split_method_matches(interp::AbstractInterpreter, argtypes::Vector{Any},
    ...
    for i in 1:length(split_argtypes)
        ...
        mt = ccall(:jl_method_cache_for, Any, (Any,), sig_n)
        mt === nothing && return FailedMethodMatch("Could not identify method cache for call")
        mt = mt::MethodCache
...
function find_simple_method_matches(interp::AbstractInterpreter, @nospecialize(atype), max_methods::Int)
    mt = ccall(:jl_method_cache_for, Any, (Any,), atype)
    if mt === nothing
        return FailedMethodMatch("Could not identify method cache for call")
    end
    mt = mt::MethodCache
